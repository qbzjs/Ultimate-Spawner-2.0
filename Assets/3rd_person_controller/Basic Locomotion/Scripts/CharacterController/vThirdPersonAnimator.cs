...
namespace Invector.vCharacterController
{
    public class vThirdPersonAnimator : vThirdPersonMotor
    {
        ...
        protected virtual void DeadAnimation()
        {
            ...
            {
                if (IsAnimatorTag("Dead"))
                {
                    AnimatorStateInfo info = (AnimatorStateInfo) animatorStateInfos.GetCurrentAnimatorStateUsingTag("Dead");

                    if (info.normalizedTime >= 0.99f && groundDistance <= 0.15f)
                        RemoveComponents();
                }
            }
